...
public class PreviewPanel extends JPanel implements Observer  {
     ...
     */
    private static final int DEFAULT_PREVIEW_WIDTH = 250;
    
    /**
     * The height of default size the preview panel(medium).
     */
    private static final int DEFAULT_PREVIEW_HEIGHT = 50;
     ...
     */
    public void drawFlags(final Graphics2D theGraphics, 
                             final int theScale) {
        ...
        for (int i = 0; i <= scale3 * scale2; i += 2 * scale3) {
            ...
            g.drawLine(getWidth() / scale1 + i, 
                    getHeight() / scale2,  getWidth() / scale1 + i , 
                    getHeight() / scale2 + 2 * scale3);
